...
export function AboutSection() {
  ...
  return (
    <section ref={sectionRef} id="about" className="py-20 px-6 bg-[var(--background)] text-[var(--foreground)] relative overflow-hidden">
      <motion.div className="absolute top-20 left-10 w-64 h-64 rounded-full bg-pink-400/5 blur-3xl" animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} />
      <motion.div className="absolute bottom-20 right-10 w-80 h-80 rounded-full bg-indigo-500/5 blur-3xl" animate={{ scale: [1, 1.3, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut', delay: 1 }} />

      <div className="w-full mx-auto px-40 relative z-10">
        <div className="flex flex-col items-center text-center mb-10">
          <motion.div ref={headingRef} className="flex items-center w-full justify-center gap-4" initial="hidden" animate={headingAnim} variants={fadeUp}>
            <motion.div className="w-110 h-0.5 bg-gradient-to-r from-transparent via-pink-400 to-pink-500" variants={headingLine} initial="hidden" animate={headingAnim} style={{ originX: 0 }} />
            <h2 className="text-4xl font-bold whitespace-nowrap">About Me</h2>
            <motion.div className="w-110 h-0.5 bg-gradient-to-l from-transparent via-pink-400 to-pink-500" variants={headingLine} initial="hidden" animate={headingAnim} style={{ originX: 1 }} />
          </motion.div>

          <motion.div ref={dotsRef} className="flex gap-2 mt-4" variants={dots} initial="hidden" animate={dotsAnim}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[60%_40%] gap-12">
          <motion.div ref={leftColumnRef} initial="hidden" animate={leftAnim} variants={fadeUp} className="relative">
            <motion.div className="absolute -top-10 -left-10 w-40 h-40 bg-gradient-to-r from-pink-400/10 to-purple-500/10 rounded-full blur-xl" animate={{ scale: [1, 1.2, 1], x: [0, 10, 0], y: [0, -10, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} />
            <div className="text-[1.325rem] leading-relaxed text-justify space-y-4">
              {sentences.map((text, i) => (
                <motion.p key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.8 }} transition={{ duration: 0.5, delay: i * 0.1 }}>
                  {text}
                </motion.p>
              ))}
            </div>
          ...
          <motion.div ref={rightColumnRef} initial="hidden" animate={rightAnim} variants={list} className="perspective-1000">
            <motion.ul className="text-[1.325rem] list-disc list-outside pl-5 space-y-4 marker:text-pink-400 text-justify">
              {interests.map(({ icon, text }, i) => (
                <motion.li key={i} variants={listItem} whileHover={{ scale: 1.02, x: 5 }} className="transform-gpu">
                  <motion.div className="p-[1px] bg-gradient-to-r from-transparent via-pink-400/30 to-transparent rounded-lg" whileHover={{ background: 'linear-gradient(90deg, transparent, rgba(236, 72, 153, 0.5), transparent)' }}>
                    <div className="p-2 rounded-lg hover:bg-white/5 transition-colors">
                      <motion.span className="text-[1.8rem] inline-block mr-2" whileHover={{ scale: 1.2, rotate: [0, -10, 10, -5, 0] }}>
                        {icon}
                      </motion.span>
                      {text}
                    </div>
            ...
            <motion.div className="absolute -bottom-20 -right-20 w-60 h-60 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-full blur-xl" animate={{ scale: [1, 1.3, 1], x: [0, -20, 0], y: [0, 20, 0] }} transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }} />
          </motion.div>
        </div>
      </div>
    </section>
